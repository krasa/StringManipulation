...
public class ColumnAlignerLine {
	...
	public ColumnAlignerLine(ColumnAlignerModel model, String textPart, boolean endsWithNextLine, String... separator) {
		...
		split = FixedStringTokenScanner.splitToFixedStringTokensAndOtherTokens(textPart, getMaxSeparatorsPerLineAsInt(model), separator).toArray(new String[0]);
		hasSeparatorBeforeFirstToken = split.length > 0 && split[0].length() == 0;
	}
	
	public Integer getMaxSeparatorsPerLineAsInt(ColumnAlignerModel model) {
		Integer integer = null;
		try {
			integer = Integer.valueOf(model.getMaxSeparatorsPerLine());
		} catch (Exception e) {
