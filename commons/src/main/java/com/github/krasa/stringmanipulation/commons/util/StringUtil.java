package com.github.krasa.stringmanipulation.commons.util;
...
public class StringUtil {
	...
	private static boolean separatorAfterDigit;
		...
		if (isEmpty(str)) {
			...
			while (index < strLen) {
				...
				if (delimiterSet.contains(codePoint)) {
					...
					int titleCaseCodePoint = toTitleCase(codePoint);
					newCodePoints[outOffset++] = titleCaseCodePoint;
					index += charCount(titleCaseCodePoint);
	...
	public static Set<Integer> generateDelimiterSet(char[] delimiters) {
		...
		if (delimiters != null && delimiters.length != 0) {
			for (int index = 0; index < delimiters.length; ++index) {
				delimiterHashSet.add(codePointAt(delimiters, index));
			}

			return delimiterHashSet;
		} else {
			if (delimiters == null) {
				delimiterHashSet.add(codePointAt(new char[]{' '}, 0));
			}
	...
	public static String toSoftCamelCase(String s) {
		...
		for (int i = 0; i < words.length; i++) {
			words[i] = IntellijStringUtil.capitalize(words[i]);
		}
	...
	public static String toCamelCase(String s) {
		...
		for (int i = 0; i < words.length; i++) {
			...
			if (firstWordNotFound && startsWithLetter(word)) {
				...
				words[i] = IntellijStringUtil.capitalize(word.toLowerCase());
			}
		}
...
//		return !isLetterOrDigit(c) && !isSlash(c) && lastOneIsNotUnderscore && !isNotBorderQuote(c, i, chars);       //replace special chars to _ (not quotes, no double _)
	...
	public static void setSeparatorAfterDigit(boolean separatorAfterDigit) {
		StringUtil.separatorAfterDigit = separatorAfterDigit;
	}

	public static void setSeparatorBeforeDigit(boolean separatorBeforeDigit) {
		StringUtil.separatorBeforeDigit = separatorBeforeDigit;
	}
}
